...
class SSK(Kernel):
    ...
    def __init__(self, seq_length: int, alphabet_size: int, gap_decay=.5, match_decay=.8,
        ...
        super(SSK, self).__init__(has_lengthscale=False, **kwargs)
        ...
        self.exp = torch.ones(self.maxlen, self.maxlen, dtype=int)
    ...
    def forward(self, X1, X2, diag=False, last_dim_is_batch=False, **params):
        ...
        # get indicies of all possible pairings from X1 and X2 ，like index
...
if __name__ == '__main__':
    ...
    str_kernel_torch = SSK(seq_length=seq_length, alphabet_size=len(alphabet), max_subsequence_length=3,
                                        gap_decay=1., match_decay=1.)
    X = torch.from_numpy(np.random.randint(0, 10, (10, 20))).to(float)
    X2 = torch.from_numpy(np.random.randint(0, 10, (10, 20))).to(float)
    ...
    str_kernel_torch = SSK(seq_length=seq_length, alphabet_size=len(alphabet), max_subsequence_length=3,
                                        gap_decay=1., match_decay=1.)
    X = torch.from_numpy(np.array([[0, 0, 1, 3, 0], [0, 2, 2, 2, 2]]).astype(float))
    X2 = torch.from_numpy(np.array([[1, 0, 1, 0, 1], [0, 1, 2, 3, 0]]).astype(float))
    kk = str_kernel_torch(X, X2).evaluate()
    print(kk)
